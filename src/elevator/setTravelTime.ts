...
export function setTravelTime(isMoving: boolean, elevatorElement: HTMLElement, nextYPosition, currentFloor: number, next): number {
  let travelTime = 0;
  if (isMoving) {
    const elevatorLaneHeight = elevatorElement.parentElement.offsetHeight;
    const currentElevPosition = elevatorElement.offsetTop;
    const newDistance = Math.abs((elevatorLaneHeight - currentElevPosition - config.building.elevatorHeight) - nextYPosition);
